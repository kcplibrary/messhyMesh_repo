...
const StatsBar = ({ stats, onFilterChange }) => {
  ...
  const yearsList = Array.from({ length: 6 }, (_, i) => String(currentRealYear - 2 + i));
  const weeksInMonthList = ["Week 1", "Week 2", "Week 3", "Week 4", "Week 5", "Week 6"];
  ...
  }, [activeTab, selectedDay, selectedWeek, selectedMonth, selectedYear, onFilterChange]);

  const totalUsers = stats.nodes || 0;
  
  // FIX: Explicitly check for null/undefined so that a mathematical 0 is treated as valid!
  const activeCount = (stats.activeFilteredCount !== null && stats.activeFilteredCount !== undefined)
  ...
  const inlineSelect = "bg-slate-900/80 border border-slate-700 text-blue-400 font-bold px-1.5 py-0.5 rounded mx-1 text-[11px] focus:outline-none cursor-pointer tracking-normal normal-case inline-block align-middle focus:border-blue-500/50 transition-colors";

  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 mb-8">
      
      {/* LEFT SIDE: CORE STAT CARDS */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center bg-slate-900/30 border border-slate-700/30 p-5 rounded-2xl h-full">
          ...
          <div className="relative w-24 h-24 sm:w-28 sm:h-28 mx-auto shrink-0">
            <svg viewBox="0 0 32 32" className="w-full h-full transform -rotate-90">
              <circle cx="16" cy="16" r="15.91549430918954" fill="transparent" className="stroke-slate-800" strokeWidth="3.5" />
              <circle cx="16" cy="16" r="15.91549430918954" fill="transparent" className="stroke-blue-500 transition-all duration-500 ease-in-out" strokeWidth="3.5" strokeDasharray={strokeDasharray} strokeDashoffset="0" />
            </svg>
          ...
          <div className="flex flex-col gap-3 justify-center pl-0 sm:pl-2 w-full">
            <div>
              ...
              <div className="text-[11px] text-slate-400 font-mono uppercase tracking-wider mt-1 leading-relaxed block">
                ...
                {activeTab === "daily" && (
                  <>
                    <span>on Day</span>
                    <select value={selectedDay} onChange={(e) => setSelectedDay(Number(e.target.value))} className={inlineSelect}>
                      {Array.from({ length: getDaysInMonth(selectedMonth, selectedYear) }, (_, i) => i + 1).map((day) => (
                        <option key={day} value={day}>{day}</option>
                      ))}
                    </select>
                    <span>of</span>
                    <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className={inlineSelect}>
                      {monthsList.map((m) => <option key={m.value} value={m.value}>{m.name}</option>)}
                    </select>
                    <select value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} className={inlineSelect}>
                ...
                {activeTab === "weekly" && (
                  <>
                    <span>during</span>
                    <select value={selectedWeek} onChange={(e) => setSelectedWeek(e.target.value)} className={inlineSelect}>
                      {weeksInMonthList.map((wk) => <option key={wk} value={wk}>{wk}</option>)}
                    </select>
                    <span>of</span>
                    <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className={inlineSelect}>
                      {monthsList.map((m) => <option key={m.value} value={m.value}>{m.name}</option>)}
            ...
            <div className="flex flex-col gap-1.5 font-mono text-[10px] border-t border-slate-800 pt-3">
              ...
              <div className="flex items-center gap-2 text-slate-500 font-medium">
                ...
                <span className="truncate">IDLE ({Math.max(0, totalUsers - activeCount)})</span>
              </div>
            </div>
          </div>
